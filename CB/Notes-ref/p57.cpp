...
//here while comparing you might have a question as to why is the second element being compared and not the first
//this is why comparator is being used inside the sort function to tell the function as to which element is to be sorted
...
int main(){
  ...
  for(int i=0; i<n ; i++){
    //initializing the first element of the pair
    pString[i].first=s[i];
    //initializing the second element of the pair
    pString[i].second=extractToken(s[i] , key);
  ...
  if(reversal=="true"){
    for(int i=0; i<n/2; i++){
      //first element of 1st pair is swapped with first element of other and similarly the second element
      swap(pString[i] , pString[n-i-1]);
    }
  }

  for(int i=0 ; i<n ; i++){
    cout<<pString[i].first<<endl;
  }
