...
class Graph{
    ...
    unordered_map<T, int> Dijkstra(T source){
        ...
        while(!s.empty()){
            ...
            for(pair<T, pair<int, int>> p: l[node]){
    ...
    int mincost(T src, T dest){
        unordered_map<string, int> distance1= Dijkstra(src);
        unordered_map<string, int> distance2= Dijkstra(dest);
        int ans= INT_MAX;
        int cur_value;
...
Problem: Given train and Flight fare for a network of cities.
         we have to find the minimum cost of travel between source to destination, given we can
         use atmost one flight.
*/  
