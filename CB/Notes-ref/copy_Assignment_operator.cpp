...
// constructor
	...
	// Default invisible one produced Shallow copy
	//let's a Deep copy
	//Here you an pass both by value and by reference
	// I didn't pass it by value because it would have an another copy of an object
	// and would have consumed extra memory
	void operator=(car &x) {
		price = x.price;
		model_no = x.model_no;
		name = new char[strlen(x.name) + 1];
